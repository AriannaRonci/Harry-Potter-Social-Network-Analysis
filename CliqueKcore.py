...
def ego_network(graph):
    ...
    nx.draw_networkx_edges(ego_graph_hp, pos, width=0.5)
...
def clique(graph):
    ...
    i = 0
    for clique in clique11:
        plt.figure(figsize=(12, 8))
        nodes = set(n for n in clique)
        h = graph.subgraph(nodes)
        deg = nx.degree(h)
        nodes = [n for n in nodes if deg[n] == 10]
        k = h.subgraph(nodes)
        k = nx.relabel_nodes(k, mapping)
        nx.draw(k, node_color="w", node_size=400, with_labels=True, font_weight='bold', font_size=15)
        i = i + 1
        plt.box(False)
        plt.savefig(f"grafici/clique11_{i}", bbox_inches="tight")
        # plt.show()
